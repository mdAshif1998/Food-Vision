...
def generate(
    ...
    with torch.no_grad():
        ...
        if do_cfg:
            ...
            # Convert into a list of length Seq_Len=77
            unconditional_tokens = tokenizer.batch_encode_plus([unconditional_prompt], padding="max_length",
            ...
            context = torch.cat([cond_context, unconditional_context])
        else:
            # Convert into a list of length Seq_Len=77
        ...
        for i, timestep in enumerate(time_steps):
            ...
            if do_cfg:
                output_cond, output_unconditional = model_output.chunk(2)
                model_output = cfg_scale * (output_cond - output_unconditional) + output_unconditional

            # (Batch_Size, 4, Latents_Height, Latents_Width) -> (Batch_Size, 4, Latents_Height, Latents_Width)
            latents = sampler.step(timestep, latents, model_output)

        dump_to_idle_device(diffusion)
